...
old3 = '\t\treturn $?\n\t}'
new3 = '\t\tlocal ret=$?\n\t\tif [ "$(uci_get_by_name $GLOBAL_SERVER type)" = "ss-libev" ]; then\n\t\t\tlocal ports="22,53,80,143,443,465,587,853,993,995,9418"\n\t\t\tnft add rule inet ss_spec ss_spec_wan_fw meta l4proto udp udp dport { $ports } counter redirect to :$local_port 2>/dev/null\n\t\t\t# UDP server bypass (mirrors TCP bypass in wan_ac; prerouting order ensures this is before the udp jump)\n\t\t\tnft add rule inet ss_spec ss_spec_prerouting iifname "br-lan" meta l4proto udp udp dport != 53 ip daddr "$server" return 2>/dev/null\n\t\t\tnft add rule inet ss_spec ss_spec_prerouting iifname "br-lan" meta l4proto udp udp dport { $ports } jump ss_spec_wan_ac comment "_SS_SPEC_RULE_" 2>/dev/null\n\t\t\t# Update persistence file immediately so auto-update and firewall restart include UDP rules\n\t\t\tnft list ruleset > /usr/share/nftables.d/ruleset-post/99-shadowsocksr.nft 2>/dev/null\n\t\tfi\n\t\treturn $ret\n\t}'
if old3 in content:
